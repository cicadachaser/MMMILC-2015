#This is an R script which analyses the data from the MMMILC Project.
...
trip<-read.csv("trip log 2015-06-03.csv",header=T,strip.white=T,na.strings= c(" ", "")) #trip log
data<-read.csv("data 2015-06-03.csv",header=T,strip.white=T,na.strings= c(" ", "")) #observations
...
data<-data[order(data$milkweed.ID,data$date),]
...
data$project.day<-data$julian.date-min(data$julian.date)+1
...
p1+geom_point(size=6,col="red")+geom_line()+geom_hline(yintercept=318,lty='dashed')+coord_cartesian(ylim = c(0, 350))+scale_x_continuous(breaks=c(1:max(trip$week)))+ylab("milkweed count from the trip log")
...
p1.1+geom_point(size=6,col="red")+geom_line()+geom_hline(yintercept=318,lty='dashed')+coord_cartesian(ylim = c(0, 350))+scale_x_continuous(breaks=c(1:max(trip$week)))+ylab("milkweed count from the data set")
...
p2+geom_point(size=6,col="blue")+geom_line()+coord_cartesian(ylim = c(0, 5))+scale_x_continuous(breaks=c(1:max(trip$week)))
